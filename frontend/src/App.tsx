...
import './App.css'; // Podpinamy nasz nowy plik ze stylami!
...
function App() {
  ...
  const [newDelivery, setNewDelivery] = useState({ fuelId: '', quantity: 1000, supplier: 'Orlen Hurt', deliveryDate: '' });
  ...
  // --- LOGOWANIE (Jedno dla wszystkich!) ---
  ...
  // Wypisywanie komunikatów błędu / sukcesu na dole ekranów
  const renderMessage = () => message && <p className={`msg ${message.includes('✅') ? 'msg-success' : 'msg-error'}`}>{message}</p>;
  ...
  if (userRole === 'owner') {
    return (
      <div className="app-container">
        <h2>Witaj, {loggedInUser}! (Panel Szefa) 💼</h2>
        ...
        <div className="card card-warning">
          ...
          <form onSubmit={handleOrderDelivery} className="flex-row">
            ...
            <input type="text" name="supplier" className="input-field input-small" placeholder="Dostawca" value={newDelivery.supplier} onChange={handleDeliveryChange} required />
        ...
        <div className="card card-info">
          ...
          <form onSubmit={handlePOSSubmit} className="flex-col">
            ...
            <div className="flex-row text-left">
              ...
              <div style={{ flex: 1 }}><label>Email (Punkty):</label><input type="email" name="customerEmail" className="input-field" value={posData.customerEmail} onChange={handlePosChange} /></div>
            </div>
    ...
    <div className="login-container">
      ...
      {loginMode === 'staff' ? (
        <form onSubmit={handleAuthSubmit} className="flex-col-sm">
          ...
          <input name="login" className="input-field" placeholder="Login pracownika lub szefa" value={staffData.login} onChange={handleStaffChange} required />
